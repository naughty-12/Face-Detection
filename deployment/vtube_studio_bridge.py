...
def try_connect_vts(vts, next_retry_at):
    now = time.perf_counter()
    if vts.connected or now < next_retry_at:
        return next_retry_at

    try:
        print(f"Connecting to VTube Studio at {vts.url} ...")
        vts.connect()
        vts.authenticate()
...
def run_bridge(args):
    ...
    vts = VTubeStudioClient(args.vtshost, args.vtsport)
    ...
    try:
        while True:
            ...
            if vts.connected and now - last_send >= send_interval:
                try:
                    ...
                    vts.close()
                    next_vts_retry = now + VTS_RETRY_INTERVAL
                last_send = now
        ...
        if vts.connected:
            try:
                vts.inject_face_position(False)
            except Exception:
                pass
        vts.close()
        cap.release()
