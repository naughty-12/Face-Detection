...
_DEFAULT_SOURCE = ROOT / "models" / "face_landmarker.task"


def _safe_path(model_path):
    """Return an ASCII-safe path for MediaPipe's C++ backend.

    MediaPipe's C++ layer uses the system locale (e.g. GBK on Chinese Windows)
    to open files, which corrupts paths containing non-ASCII characters.
    When the resolved path contains non-ASCII, the model is copied to a temp
    ...
    try:
        ...
        tmp_dir = Path(tempfile.gettempdir()) / "mediapipe_models"
        tmp_dir.mkdir(exist_ok=True)
        tmp_model = tmp_dir / "face_landmarker.task"
        if not tmp_model.exists() or tmp_model.stat().st_size != Path(model_path).stat().st_size:
            shutil.copy2(model_path, tmp_model)
        return str(tmp_model)
    return resolved


# Expose the default model via the safe-path helper so it works even when the
# project lives under a path with CJK / non-ASCII characters.
DEFAULT_MODEL = _safe_path(_DEFAULT_SOURCE)
...
class MediaPipeFaceLandmarker:
    ...
    def __init__(self, model_path=DEFAULT_MODEL, crop_scale=1.45):
        safe_path = _safe_path(model_path)
        self.model_path = Path(safe_path)
        if not self.model_path.exists():
            raise FileNotFoundError(f"MediaPipe Face Landmarker model not found: {self.model_path}")
        self._tmp_dir = os.path.commonpath([safe_path, tempfile.gettempdir()]) == tempfile.gettempdir()
