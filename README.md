...
|:---|---:|---:|---:|:---|---:|
| **PyTorch `.pt`** | 0.66404 | 0.35618 | 87.8 – 147 FPS | **CUDA（GPU）** | 5.95 MiB |
| ONNX **FP16** | 0.66382 | 0.35627 | ~36 FPS | CPU | **5.88 MiB** |
| ONNX FP32 | 0.66379 | 0.35670 | ~36 FPS | CPU | 11.70 MiB |
...
>
> ⚠️ **速度口径**：PyTorch 一栏实测波动明显（同一命令测到 6.9 / 9.2 / 11.4 ms，折合 88–147 FPS），
> 受 GPU 状态与占用影响。引用时请说明是单次测量还是多次中位数，不要把区间端点当固定值。
...
| 小脸召回 | 汇总召回 0.610，其中 **72.5% 的 GT 人脸 <32px、召回仅 0.493**；≥32px 人脸召回 0.916。尚无 conf 阈值扫描数据 |
...
| 桥接默认模型 | 桥接默认加载 `.onnx`，因此**当前跑在 CPU 上、GPU 闲置**（见下方建议） |

### 运行方式建议（本机实测）

| 场景 | 建议 | 原因 |
|:---|:---|:---|
| **日常使用、追求速度** | **PyTorch `.pt` + CUDA** | 87.8–147 FPS，比 ONNX 快 2.4–3.3 倍 |
| 免装 PyTorch 的部署 | ONNX **FP16** | 5.88 MiB，mAP 与 PyTorch 无差别，但只能跑 CPU |
| **VTube Studio 桥接** | **把默认模型改为 `.pt`** | 现默认 `.onnx` → CPU 推理；`--model` 指定 `.pt` 即可用上 GPU |
